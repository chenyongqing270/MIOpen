...
def buildHipClangJob(compiler, flags, env4make, image, prefixpath="/opt/rocm", cmd = ""){
        ...
        env.HSA_ENABLE_SDMA=0 
        checkout scm
        def dockerOpts="-u 0 --device=/dev/kfd --device=/dev/dri --group-add video --cap-add=SYS_PTRACE --security-opt seccomp=unconfined"
                ...
                stage('Hip Implicitgemm layouts Release') {
                    ...
                    environment{
                        cmd = '''
                            ...
                            dir=$(pwd)
                            cd
                            git clone -b zyin-test https://github.com/ROCmSoftwarePlatform/llvm-project-mlir.git
